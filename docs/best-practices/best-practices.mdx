...
If you’re a developer who wants to start managing and deploying applications in a continuous and streamlined way, DevCycle is the tool for you!
...
We will create our React app using the terminal. Open your computer’s terminal, and using the `cd` command, open the directory where you would like to create your app. Now enter the following command:
```jsx
...
Open a new window of your terminal (make sure to keep open the window that is running your React app). `cd` into the directory where you created your React app and open our project:
```jsx 
cd getting-started-devcycle 
```
```jsx
...
To install the React SDK, run the following command on your terminal `npm install --save @devcycle/devcycle-react-sdk`

Next, we import DevCycle and initialize it. An example usage of our feature flag is conveniently located in the “**Example Usage**” section on your feature’s DevCycle page. This gives us the code we need to import and initialize DevCycle. All we need to do is select the environment we’re initializing for (Development), the variable we’re using (first-feature), and the SDK we’re initializing it with (Client - React). DevCycle then provides us with the code we need, along with our unique SDK Key, to access our feature flag in our application. The provided code for our particular feature flag will look something like this, with `YOUR_CLIENT_KEY_HERE` replaced with your actual client key:
...
Copy the code above (or the one provided in the “Example Usage” section of your feature flag’s page), and use it to replace the whole `function App () {...}` section in VSCode. You should now have something that looks like this:

![Pasted from example usage](/june-2022-example-usage-code-react.jpeg)

Notice how line 8 includes your private key. **A best practice is to instead use a constant variable instead whenever you use your key.** That way, we avoid any typo errors as we use our key throughout the code. After line 4, add the following:

```jsx
const ENV_KEY = 'YOUR_KEY_HERE'
```

Replace `YOUR_KEY_HERE` with your key, and use the new `ENV_KEY` variable whenever you need to access your key.

There are a few more things we need to modify if you’re using React 18. Add the following to the beginning of your code:
```jsx
...
Another way to see our flag working is to toggle the Targeting switch on the feature page in DevCycle. If we turn our targeting toggle off and save it, we won’t be able to see our feature regardless of our email address.

![Targeting Off Toggle](/june-2022-targeting-toggle.gif)
