...
Feature Opt-In provides the ability to make any Feature released via DevCycle have a self-serve way to enable it for end users. This all happens through a simple iFrame that can be implemented on your website, which is powered by DevCycle.

This means that if you are looking to run a beta program but don't want to manually manage access to beta Features, you can simply add those Features to the Feature Opt-In widget and your users can toggle them on or off as they wish.
...
Feature Opt-In is first enabled at the Project-level. To enable Feature Opt-In, switch the toggle to "Opt-In Enabled".
...
This special Targeting Rule allows teams to be explicit about what happens when a user enables a Feature for themselves. It also allows you to set overrides if you would like, based on the priority order of Targeting Rules and where the Opt-In Targeting Rule sits.
